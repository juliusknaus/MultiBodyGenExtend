...
class DexGripperStackingEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, cfg, agent):
        ...
        if self.cfg.xml_name == "default":
            self.model_xml_file = os.path.join(cfg.project_path, "assets", "mujoco_envs", "dex_gripper_stacking.xml")
        else:
        ...
        self.box1_body_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "box_1"
        )
        self.box2_body_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "box_2"
        )

        self.box1_geom_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, "box_1"
        )
        self.box2_geom_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, "box_2"
        )

        self.box1_joint_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_JOINT, "box_1_joint"
        )
        self.box2_joint_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_JOINT, "box_2_joint"
        )

        self.box1_qpos_adr = self.model.jnt_qposadr[self.box1_joint_id]
        self.box2_qpos_adr = self.model.jnt_qposadr[self.box2_joint_id]

        self.box1_radius = self.model.geom_size[self.box1_geom_id][0]
        self.box2_radius = self.model.geom_size[self.box2_geom_id][0]

        self.box1_init_height = self.box1_radius
        self.box2_init_height = self.box2_radius
        ...
        self.target_pos = self.data.xpos[self.box2_body_id]
        self.target_height = cfg.task_specs.get('target_height', {})
        



        # geom id (first geom attached to that body)
        geom_start = self.model.body_geomadr[self.box1_body_id]
        if self.obj_name == 'sphere' or self.obj_name == 'cylinder' or self.obj_name == 'capsule' or self.obj_name == 'box':
            geom_num   = self.model.body_geomnum[self.box1_body_id]
    ...
    def staged_rewards(self):
        ...
        # Object positions
        # -------------------------
        boxA_pos = self.data.geom_xpos[self.obj_geom_id].copy()
        boxB_pos = self.data.geom_xpos[self.target_geom_id].copy()
        ...
        # Lift reward
        # -------------------------
        boxA_bottom_z = self.compute_object_lowest_point(
           # self.model,
           # self.data,
            self.obj_geom_id
        )
        ...
        boxA_touching_boxB = self.compute_box_gripper_contacts(
            self.model,
            self.data,
            [self.target_geom_id],   # if you have a contact helper for box-box, use that instead
            self.obj_geom_id
        ) > 0
    ...
    def is_gripper_contact(self, contact, limb_geom_ids):
        return (
            ...
            or
            (contact.geom2 in limb_geom_ids and contact.geom1 == self.obj_geom_id)
        )
    ...
    def occupancy_in_gripper(self, box_center, box_size, tri, n_samples=500):
        ...
        samples = self.sample_points_in_object(box_center, self.obj_geom_id, n_samples)
    ...
    def gripper_box_overlap(self, box_pos, box_size, tri, n_samples=500):
        ...
        samples = self.sample_points_in_object(box_pos, self.obj_geom_id, n_samples)
    ...
    def compute_object_lowest_point(self, geom_id):
        ...
        if self.obj_name == 'sphere':
            ...
            # Composite cup: take the minimum z over all cup geoms,
            # including the capsule handle if it exists.
            lowest_z = np.inf

            for gid in self.obj_geom_ids:
                gtype = self.model.geom_type[gid]
                gsize = self.model.geom_size[gid]
                gcenter = self.data.geom_xpos[gid].copy()
                R = self.data.geom_xmat[gid].reshape(3, 3).copy()
                ...
                if gtype == mujoco.mjtGeom.mjGEOM_SPHERE:
                    ...
                    corners_world = (corners_local @ R.T) + gcenter
                    zmin = np.min(corners_world[:, 2])

                elif gtype == mujoco.mjtGeom.mjGEOM_CYLINDER:
                    radius, half_h = gsize[0], gsize[1]
                    pts_local = np.array([
                        [ radius, 0.0, -half_h],
                        [-radius, 0.0, -half_h],
                        [0.0,  radius, -half_h],
                    ...
                    pts_world = (pts_local @ R.T) + gcenter
                    zmin = np.min(pts_world[:, 2])

                elif gtype == mujoco.mjtGeom.mjGEOM_CAPSULE:
                    radius, half_h = gsize[0], gsize[1]
                    pts_local = np.array([
                        [0.0, 0.0, -(half_h + radius)],
                        [0.0, 0.0,  (half_h + radius)],
                        [ radius, 0.0, -half_h],
                        [-radius, 0.0, -half_h],
                        [0.0,  radius, -half_h],
    ...
    def get_sim_obs(self):
        ...
        for i, body in enumerate(self.robot.bodies):
            ...
            if i == 0:
                ...
                qs, qe = get_single_body_qposaddr(
                    self.model,
                    body.name
                )
                ...
                if qs < 0 or qe > self.model.nq or qs > qe:
                    raise RuntimeError(
                        f"Invalid qpos range for {body.name}"
                    )

                if (qs - 1) < 0 or (qe - 1) > self.model.nv:
                    raise RuntimeError(
                        f"Invalid qvel range for {body.name}"
                    )
        ...
        # box 1
        box1_pos = self.data.qpos[
            self.box1_qpos_adr:
            self.box1_qpos_adr + 3
        ]

        box1_quat = self.data.qpos[
            self.box1_qpos_adr + 3:
            self.box1_qpos_adr + 7
        ]

        box1_vel = qvel[
            self.box1_qvel_adr:
            self.box1_qvel_adr + 6
        ]

        # box 2
        box2_pos = self.data.qpos[
            self.box2_qpos_adr:
            self.box2_qpos_adr + 3
        ]

        box2_quat = self.data.qpos[
            self.box2_qpos_adr + 3:
            self.box2_qpos_adr + 7
        ]

        box2_vel = qvel[
            self.box2_qvel_adr:
            self.box2_qvel_adr + 6
        ]
        ...
        gripper_pos = self.get_body_com("0")[:3]
