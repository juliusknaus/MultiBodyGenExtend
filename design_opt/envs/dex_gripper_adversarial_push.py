...
class DexGripperAdversarialPushEnv(MujocoEnv, utils.EzPickle):
    ...
    def _get_agent_root_obs(self, agent_id):
        ...
        if x_joint_id != -1 and y_joint_id != -1 and z_joint_id != -1:
            ...
            return [
                ...
                self.data.qvel[x_qvel_adr:x_qvel_adr + 6],
                np.zeros(2),
            ]
    ...
    def action_to_control(self, a, agent_id):
        ...
        # print("root_ids: ", root_ids, " and other_ids: ", other_ids, " and a.shape: ", a.shape)
    ...
    def multi_step(self, joint_action, agent_id=None):
        ...
        if self._all_agents_in_execution():
            ...
            for idx in range(self.num_agents):
                ...
                idle_box_progress_thresh = float(self.cfg.reward_specs.get('idle_box_progress_thresh', 5e-4))
                ...
                reward_components['idle'] = float(idle_penalty_component)

                if self._has_goal_lines:
                    box_x_bef = float(box_pos_bef[0])
                    box_x_aft = float(box_pos_aft[0])

                    goal_line_dist_bef = self._goal_line_distance(idx, box_x_bef)
                    ...
                    own_crossed_after = self._goal_line_crossed(idx, box_x_aft)
                    opponent_id = 1 - idx if self.num_agents > 1 else None
                    opponent_crossed_before = False
                    opponent_crossed_after = False
                    opponent_goal_line_component = 0.0
                    if opponent_id is not None:
                        opponent_crossed_before = self._goal_line_crossed(opponent_id, box_x_bef)
                        opponent_crossed_after = self._goal_line_crossed(opponent_id, box_x_aft)
                        opponent_goal_line_dist_bef = self._goal_line_distance(opponent_id, box_x_bef)
                        opponent_goal_line_dist_aft = self._goal_line_distance(opponent_id, box_x_aft)
                        opponent_goal_line_dist_coeff = float(self.cfg.reward_specs.get('opponent_goal_line_dist_coeff', 1.0))
                        opponent_goal_line_component = -opponent_goal_line_dist_coeff * (
                            (opponent_goal_line_dist_bef - opponent_goal_line_dist_aft) / self.dt
                        )
                        reward += opponent_goal_line_component

                    goal_bonus = float(self.task_specs.get('goal_bonus', 10.0))
                    goal_penalty = float(self.task_specs.get('goal_penalty', goal_bonus))
                    goal_event_coeff = float(self.cfg.reward_specs.get('goal_event_coeff', 1.0))
                    goal_event_component = goal_event_coeff * (
                        (goal_bonus if ((not own_crossed_before) and own_crossed_after) else 0.0)
                        - (goal_penalty if (opponent_id is not None and (not opponent_crossed_before) and opponent_crossed_after) else 0.0)
                    )
                    reward += goal_event_component
                    reward_components['goal'] = float(goal_event_component)
                    reward_components['goal_line_distance'] = float(goal_line_distance_component)
                    reward_components['opponent_goal_line'] = float(opponent_goal_line_component)
    ...
    def step(self, a, agent_id=0):
        ...
        # execution stage
            ...
            # Core shaping reward. Previously this was overwritten to distance-only,
            # which made standing still a common local optimum.
            reward_weights = self.task_specs.get('weights', {})
            reward = (
            ...
            # Penalize no-op behavior: if both box progress and approach progress are tiny,
            # apply a small negative reward so standing still is not attractive.
            box_progress = abs(float(box_pos_aft[0]) - float(box_pos_bef[0]))
            approach_progress = abs(float(rob_box_dist_bef - rob_box_dist_aft))
            idle_box_progress_thresh = float(self.cfg.reward_specs.get('idle_box_progress_thresh', 5e-4))
            idle_approach_progress_thresh = float(self.cfg.reward_specs.get('idle_approach_progress_thresh', 5e-4))
            idle_penalty_coeff = float(self.cfg.reward_specs.get('idle_penalty_coeff', 0.02))
            idle_penalty_component = 0.0
            ...
            if self._has_goal_lines:
                ...
                # Per-agent shaping: reward movement that brings the box
                # closer to the current agent's own goal line.
                goal_line_dist_bef = self._goal_line_distance(agent_id, box_x_bef)
                ...
                own_crossed_after = self._goal_line_crossed(agent_id, box_x_aft)
                opponent_id = 1 - agent_id if self.num_agents > 1 else None
                opponent_crossed_before = False
                opponent_crossed_after = False
                opponent_goal_line_component = 0.0
                if opponent_id is not None:
                    ...
                    opponent_goal_line_dist_aft = self._goal_line_distance(opponent_id, box_x_aft)
                    opponent_goal_line_dist_coeff = float(self.cfg.reward_specs.get('opponent_goal_line_dist_coeff', 1.0))
                    # Zero-sum shaping: penalize helping the opponent's line objective.
                    opponent_goal_line_component = -opponent_goal_line_dist_coeff * (
                        (opponent_goal_line_dist_bef - opponent_goal_line_dist_aft) / self.dt
                    )
                    reward += opponent_goal_line_component
                ...
                # Make the adversarial event term part of the optimized reward,
                # not just a logged diagnostic.
                reward += goal_event_component
                reward_components['goal'] = float(goal_event_component)
                reward_components['goal_line_distance'] = float(goal_line_distance_component)
                reward_components['opponent_goal_line'] = float(opponent_goal_line_component)
    ...
    def reset_state(self, add_noise, agent_id=0):
        ...
        # Optional root overrides (shared vector or per-agent matrix).
        root_init_pos = self.task_specs.get('root_init_pos', None)
        if root_init_pos is not None:
            root_init_pos = np.asarray(root_init_pos, dtype=np.float64)
            if root_init_pos.ndim not in (1, 2):
                raise ValueError("task_specs['root_init_pos'] must be 1D or 2D")
            if root_init_pos.ndim == 1 and root_init_pos.shape[0] != 3:
                raise ValueError("task_specs['root_init_pos'] 1D value must have length 3")
            if root_init_pos.ndim == 2 and root_init_pos.shape[1] != 3:
                raise ValueError("task_specs['root_init_pos'] 2D value must have shape (N, 3)")

        root_init_xy = self.task_specs.get('root_init_xy', None)
        if root_init_xy is not None:
            root_init_xy = np.asarray(root_init_xy, dtype=np.float64)
            if root_init_xy.ndim not in (1, 2):
                raise ValueError("task_specs['root_init_xy'] must be 1D or 2D")
            if root_init_xy.ndim == 1 and root_init_xy.shape[0] != 2:
                raise ValueError("task_specs['root_init_xy'] 1D value must have length 2")
            if root_init_xy.ndim == 2 and root_init_xy.shape[1] != 2:
                raise ValueError("task_specs['root_init_xy'] 2D value must have shape (N, 2)")

        root_init_quat = self.task_specs.get('root_init_quat', None)
        if root_init_quat is not None:
            root_init_quat = np.asarray(root_init_quat, dtype=np.float64)
            if root_init_quat.ndim not in (1, 2):
                raise ValueError("task_specs['root_init_quat'] must be 1D or 2D")
            if root_init_quat.ndim == 1 and root_init_quat.shape[0] != 4:
                raise ValueError("task_specs['root_init_quat'] 1D value must have length 4")
            if root_init_quat.ndim == 2 and root_init_quat.shape[1] != 4:
                raise ValueError("task_specs['root_init_quat'] 2D value must have shape (N, 4)")

        # Support both root architectures used across env XMLs:
        # 1) free joint root_i
        # 2) three slide joints root_x_i/root_y_i/root_z_i
        #
        # If this env uses free roots at origin and no explicit XY override is
        # provided, fall back to multi-grasping-style spawn XY so agents do not
        # initialize at (0, 0).
        root_init_z_cfg = self.task_specs.get('root_init_z', None)
        default_root_init_xy_cfg = self.task_specs.get('default_root_init_xy', [[-2.0, -2.0], [2.0, 2.0]])
        ...
        if default_root_init_xy.ndim != 2 or default_root_init_xy.shape[1] != 2:
            raise ValueError(
                "task_specs['default_root_init_xy'] must be shape (N, 2)"
            )

        for i in range(self.num_agents):
            root_init_pos_i = None
            if root_init_pos is not None:
                if root_init_pos.ndim == 1:
                    root_init_pos_i = root_init_pos.copy()
                else:
                    row = min(i, root_init_pos.shape[0] - 1)
                    root_init_pos_i = root_init_pos[row].copy()

            root_init_xy_i = None
            if root_init_xy is not None:
                if root_init_xy.ndim == 1:
                    root_init_xy_i = root_init_xy.copy()
                else:
                    row = min(i, root_init_xy.shape[0] - 1)
                    root_init_xy_i = root_init_xy[row].copy()

            root_init_quat_i = None
            if root_init_quat is not None:
                if root_init_quat.ndim == 1:
                    root_init_quat_i = root_init_quat.copy()
                else:
                    row = min(i, root_init_quat.shape[0] - 1)
                    root_init_quat_i = root_init_quat[row].copy()

            default_xy_row = min(i, default_root_init_xy.shape[0] - 1)
            default_xy_i = default_root_init_xy[default_xy_row]

            # -------- free-joint root path --------
            ...
            if root_joint_id != -1 and self.model.jnt_type[root_joint_id] == mujoco.mjtJoint.mjJNT_FREE:
                ...
                # Freejoint world translation is stored in qpos, not body_pos.
                root_pos = qpos[qpos_adr:qpos_adr + 3].copy()

                # Full position override has priority; otherwise XY override.
                if root_init_pos_i is not None:
                    root_pos[:] = root_init_pos_i
                elif root_init_xy_i is not None:
                    root_pos[:2] = root_init_xy_i
                else:
                    # No explicit XY override: avoid origin collapse by applying
                    # multi-grasping-style default XY for this env.
                    root_pos[:2] = default_xy_i

                if root_init_z_cfg is not None:
                    if isinstance(root_init_z_cfg, (list, tuple, np.ndarray)):
                        if i < len(root_init_z_cfg):
                            root_pos[2] = float(root_init_z_cfg[i])
                    else:
                        root_pos[2] = float(root_init_z_cfg)

                qpos[qpos_adr:qpos_adr + 3] = root_pos
                if root_init_quat_i is not None:
                    qpos[qpos_adr + 3:qpos_adr + 7] = root_init_quat_i
                else:
                    qpos[qpos_adr + 3:qpos_adr + 7] = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64)
                qvel[qvel_adr:qvel_adr + 6] = 0.0
                continue

            # -------- slide-joint root path --------
            ...
            if x_joint_id == -1 or y_joint_id == -1 or z_joint_id == -1:
                # Fallback: resolve root joints from the agent root body rather than
                # relying on specific joint names.
                ...
                # Identify x/y/z slide joints by dominant axis component.
                comp_to_joint = {}
                for jid in slide_joint_ids:
                    axis = self.model.jnt_axis[jid]
                    axis_comp = int(np.argmax(np.abs(axis)))
                ...
                if 0 in comp_to_joint and 1 in comp_to_joint and 2 in comp_to_joint:
                    x_joint_id = comp_to_joint[0]
                    y_joint_id = comp_to_joint[1]
                    z_joint_id = comp_to_joint[2]
                else:
                    # Fallback to qpos order when axis-based mapping is ambiguous.
            ...
            qpos[x_qpos_adr] = root_pos[0]
            qpos[y_qpos_adr] = root_pos[1]
            qpos[z_qpos_adr] = root_pos[2]
            qvel[x_qvel_adr] = 0.0
            qvel[y_qvel_adr] = 0.0
            qvel[z_qvel_adr] = 0.0

        # Initialize the box pose explicitly; otherwise the multi-grasping
        # env keeps the XML default pose instead of task_specs['box_pos'].
        ...
        root_body_id = self._get_agent_root_body_id(0)
